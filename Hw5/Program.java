...
public class Program {
   ...
   *This program was edited by Nyla Worker. 
   */
      ...
      while(mScanner.hasNextLine()){
        ...
        for(int i = 0; i < line.length(); i++){
          ...
          if (prevChar == '/' && character == '*'){
            ...
            char prevchartem = 'w';
            do{ 
              prevchartem = charactertem;
              for(int j = 0 ; j < line.length() ; j++){
                charactertem = line.charAt(j);
              }
            }while(prevchartem ==  '*' && charactertem == '/');
          }   
  ...
  private void loadMethodList(Scanner mScanner) {
    ...
    mScanner = new Scanner(sb.toString());
    while(mScanner.hasNextLine()) {
